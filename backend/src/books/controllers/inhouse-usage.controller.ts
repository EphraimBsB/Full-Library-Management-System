...
export class InhouseUsageController {
  ...
  @ApiResponse({
    status: 201,
    description: 'In-house usage started',
    type: InhouseUsageResponseDto,
  })
  async startUsage(
    @Request() req,
    @Body() startUsageDto: StartInhouseUsageDto,
  ): Promise<{ message: string; statusCode: number; usageId: string }> {
    const result = await this.inhouseUsageService.startUsage(
      req.user.id,
      startUsageDto,
    );
    return {
      message: 'In-library reading started successfully',
      statusCode: 201,
      usageId: result.id,
    };
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'In-house usage ended',
    type: InhouseUsageResponseDto,
  })
  async endUsage(
    @Request() req,
    ...
    const result = await this.inhouseUsageService.endUsage(
      usageId,
      req.user.id,
    );
    return {
      message: 'In-library reading ended successfully',
      statusCode: 200,
      usageId: result.id,
    };
  }
  ...
  @ApiOperation({
    summary:
      'Force end an active in-house usage session (Admin/Librarian only)',
  })
  @ApiResponse({
    status: 200,
    description: 'The usage has been force ended',
    type: InhouseUsageResponseDto,
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiResponse({
    status: 400,
    description: 'Usage is not active',
    type: InhouseUsageResponseDto,
  })
  async forceEndUsage(
    @Param('id') id: string,
    ...
    return {
      message: 'In-library reading force ended successfully',
      statusCode: 200,
      usageId: result.id,
    };
  }
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        items: {
          type: 'array',
          items: { $ref: getSchemaPath(InhouseUsageResponseDto) },
        },
        total: { type: 'number' },
      },
    },
  ...
  @ApiResponse({
    status: 200,
    description: 'Paginated in-house usage history',
    type: [InhouseUsageResponseDto],
  })
  async getUsageHistory(
    ...
    return this.inhouseUsageService.getUserUsageHistory(
      req.user.id,
      limit,
      offset,
      status,
