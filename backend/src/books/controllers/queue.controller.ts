...
export class QueueController {
  ...
  async joinQueue(@Param('bookId') bookId: string, @Req() req: any) {
  ...
  @ApiResponse({
    status: 409,
    description: 'Cannot leave queue in current state',
  })
  async leaveQueue(@Param('entryId') entryId: string, @Req() req: any) {
    const userId = req.user.id;
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Queue entry not found' })
  async getQueuePosition(@Param('bookId') bookId: string, @Req() req: any) {
    const userId = req.user.id;
  ...
  @ApiResponse({
    status: 200,
    description: "Returns list of user's queue entries",
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
