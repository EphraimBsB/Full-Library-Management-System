...
import {
  BookFavoriteResponseDto,
  ToggleBookFavoriteDto,
} from '../dto/book-favorite.dto';
...
export class BookFavoriteController {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiOperation({
    summary: 'Get all favorite books for the authenticated user',
  })
  @ApiResponse({
    status: 200,
  ...
  async getUserFavorites(@Request() req): Promise<Book[]> {
  ...
  async checkFavorite(
    ...
    const isFavorite = await this.bookFavoriteService.isBookFavorite(
      req.user.id,
      bookId,
    );
    return { isFavorite };
