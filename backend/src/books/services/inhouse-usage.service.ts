...
import {
  BookInhouseUsage,
  InhouseUsageStatus,
} from '../entities/book-inhouse-usage.entity';
...
export class InhouseUsageService {
    ...
    if (copyId) {
      ...
      if (!copy) {
        throw new NotFoundException(
          `Copy with ID ${copyId} not found for this book`,
        );
      }
    }
  ...
  async endUsage(
    ...
    if (usage.status !== InhouseUsageStatus.ACTIVE) {
      throw new BadRequestException(
        `This usage session has already been ${usage.status}`,
      );
    }

    usage.endedAt = new Date();
    usage.durationMinutes = Math.floor(
      (usage.endedAt.getTime() - usage.startedAt.getTime()) / (1000 * 60),
    );
    usage.status = isForced
      ? InhouseUsageStatus.FORCE_ENDED
      : InhouseUsageStatus.COMPLETED;
  ...
  async getAllUsages(
    ...
    const qb = this.inhouseUsageRepository
      .createQueryBuilder('inhouseUsage')
