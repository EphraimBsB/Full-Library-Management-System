...
import {
  BookCopy,
  BookCopyStatus,
} from '../../../books/entities/book-copy.entity';
...
export class BookLoansSeed implements ISeeder {
  public async run(dataSource: DataSource): Promise<SeedResult> {
    ...
    if (memberships.length === 0) {
      console.warn(
        'No active memberships found. Please seed memberships first.',
      );
    ...
    if (availableCopies.length === 0) {
      console.warn(
        'No available book copies found. Please seed book copies first.',
      );
      return { entity: 'BookLoan', count: 0 };
    }
      ...
      for (let i = 0; i < loanCount && availableCopies.length > 0; i++) {
        ...
        if (status === LoanStatus.RETURNED) {
          ...
          const weeksOverdue = Math.min(
            Math.ceil(
              (now.getTime() - dueDate.getTime()) / (7 * 24 * 60 * 60 * 1000),
            ),
            4, // Cap at 4 weeks
          );
          ...
          if (weeksOverdue > 0) {
            ...
            const bookPrice = priceResult?.price
              ? parseFloat(priceResult.price)
              : 20; // Default to $20 if price not set
            const finePerWeek =
              bookPrice * (0.01 * (1 + Math.floor(Math.random() * 10))); // 1-10% of book price
            loan.fineAmount = parseFloat(
              (weeksOverdue * finePerWeek).toFixed(2),
            );
          }
    ...
    for (let i = 0; i < loans.length; i += batchSize) {
      ...
      try {
        ...
        console.log(
          `Processed batch of ${batch.length} book loans (${Math.min(i + batch.length, loans.length)}/${loans.length})`,
        );
      } catch (error) {
