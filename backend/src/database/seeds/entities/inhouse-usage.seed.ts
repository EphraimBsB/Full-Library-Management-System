...
import {
  BookInhouseUsage,
  InhouseUsageStatus,
} from '../../../books/entities/book-inhouse-usage.entity';
...
export class InhouseUsageSeed implements ISeeder {
  ...
  async run(dataSource: DataSource): Promise<SeedResult> {
    ...
    if (books.length === 0 || users.length === 0) {
      console.warn(
        'Not enough books or users found. Please seed books and users first.',
      );
    ...
    for (let i = 0; i < usageCount; i++) {
      ...
      const copy =
        copies.length > 0 ? faker.helpers.arrayElement(copies) : null;
    ...
    const savedUsages = await dataSource.transaction(
    ...
    console.log(
      `[${this.name}] Successfully seeded ${savedUsages.length} in-house usage records`,
    );
