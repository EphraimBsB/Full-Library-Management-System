...
export class BookRequestsSeed implements ISeeder {
  public async run(dataSource: DataSource): Promise<SeedResult> {
    ...
    if (books.length === 0) {
      console.warn(
        'No books with low availability found. Please seed books first.',
      );
    ...
    for (let i = 0; i < requests.length; i += batchSize) {
      ...
      try {
        ...
        console.log(
          `Processed batch of ${batch.length} book requests (${Math.min(i + batch.length, requests.length)}/${requests.length})`,
        );
      } catch (error) {
    ...
    return {
      ...
      count: created,
      details: {
        pending: requests.filter((r) => r.status === BookRequestStatus.PENDING)
          .length,
        approved: requests.filter(
          (r) => r.status === BookRequestStatus.APPROVED,
        ).length,
        fulfilled: requests.filter(
          (r) => r.status === BookRequestStatus.FULFILLED,
        ).length,
        rejected: requests.filter(
          (r) => r.status === BookRequestStatus.REJECTED,
        ).length,
      },
    };
  }
