...
export class ShelvesSeed implements ISeeder {
  public async run(dataSource: DataSource): Promise<SeedResult> {
    ...
    const mainCampus = await locationRepository.findOne({ where: { name: 'Main Campus Library' } });
    const cityCampus = await locationRepository.findOne({ where: { name: 'City Campus Library' } });
    const scienceLibrary = await locationRepository.findOne({ where: { name: 'Science & Technology Library' } });
    const medicalLibrary = await locationRepository.findOne({ where: { name: 'Medical Campus Library' } });
    const distanceCenter = await locationRepository.findOne({ where: { name: 'Distance Learning Center' } });
