  ...
  private generateCopyData(
    ...
    return {
      ...
      notes: [
        ...
        faker.helpers.maybe(() => `Note: ${faker.lorem.sentence()}`, {
          probability: 0.3,
        }),
      ]
        .filter(Boolean)
      ...
      for (const book of batch) {
        try {
          ...
          if (existingCopies.length === totalCopies) {
            results.push(
              `Skipping book ID ${book.id}: Already has ${totalCopies} copies`,
            );
            continue;
          }

          // Calculate how many new copies to create
          const newCopiesNeeded = Math.max(
            0,
            totalCopies - existingCopies.length,
          );
          ...
          for (let i = 0; i < Math.min(availableToAdd, newCopiesNeeded); i++) {
            ...
            const copyData = this.generateCopyData(
              book,
              copyNumber,
              'available',
            );
            ...
            const newCopy = bookCopyRepository.create({
              ...
              accessNumber: this.generateAccessNumber(
                book.id.toString(),
                copyNumber,
              ),
          ...
          if (remainingToAdd > 0) {
            ...
            for (let i = 0; i < remainingToAdd; i++) {
              ...
              const copyData = this.generateCopyData(
                book,
                copyNumber,
                status.toLowerCase() as CopyStatus,
              );
          ...
          if (newCopies.length > 0) {
            ...
            results.push(
              `Added ${newCopies.length} copies for book ID ${book.id}: "${book.title}"`,
            );
          }
