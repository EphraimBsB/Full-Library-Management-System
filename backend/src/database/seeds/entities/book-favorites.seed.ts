...
export class BookFavoritesSeed implements ISeeder {
  public async run(dataSource: DataSource): Promise<SeedResult> {
    ...
    if (books.length === 0 || users.length === 0) {
      console.warn(
        'Not enough books or users found to create favorites. Skipping...',
      );
    ...
    try {
      ...
      const uniqueFavorites = Array.from(
        new Map(
          favorites.map((fav) => [`${fav.userId}-${fav.bookId}`, fav]),
        ).values(),
      );

      await bookFavoriteRepository.save(uniqueFavorites);
      console.log(
        `Successfully seeded ${uniqueFavorites.length} book favorites`,
      );
  ...
  private async updateFavoriteCounts(
    ...
    for (const book of books) {
      ...
      if (bookWithMeta && bookWithMeta.metadata) {
        ...
        const favoriteCount = await dataSource
          .getRepository(BookFavorite)
          .count({
